...
def admins(inp, notice=None, bot=None, chan=None, db=None):
    ...
    if admins: notice("[{}]: Admins are: {}".format(chan, admins))
    else: notice("[{}]: No nicks/hosts are currently admins.".format(chan))
    return
    ...
    if 'add' in command:
        for nick in nicks:
            ...
            if admins and nick in admins:
                notice("[{}]: {} is already an admin.".format(chan, nick))
            else:
                admins = '{} {}'.format(nick, admins).replace('False',
                                                              '').strip()
                database.set(db, 'channels', 'admins', admins, 'chan', chan)
                notice("[{}]: {} is now an admin.".format(chan, nick))
    elif 'del' in command:
        if '*' in nicks:
            database.set(db, 'channels', 'admins', '', 'chan', chan)
            notice("[{}]: All admins have been removed.".format(chan))
        else:
            for nick in nicks:
                nick = user.get_hostmask(nick, db)
                if admins and nick in admins:
                    ...
                    database.set(db, 'channels', 'admins', admins, 'chan',
                                 chan)
                    notice("[{}]: {} is no longer an admin.".format(
                        chan, nick))
                else:
                    notice("[{}]: {} is not an admin.".format(chan, nick))
    return
    ...
    if 'enable' in command:
        ...
        for nick in nicks:
            ...
            if autoops and nick in autoops:
                ...
                database.set(db, 'channels', 'autoops', autoops, 'chan', chan)
                notice("[{}]: {} is no longer an auto op.".format(
                    chan, nick))
            else:
                notice("[{}]: {} is not an auto op.".format(chan, nick))
    return
...
def ignored(inp, notice=None, bot=None, chan=None, db=None):
    ...
    if ignorelist:
        notice("[{}]: Ignored nicks/hosts are: {}".format(chan, ignorelist))
    else:
        notice("[{}]: No nicks/hosts are currently ignored.".format(chan))
    return
...
def ignore(inp, notice=None, bot=None, chan=None, db=None):
    ...
    for target in targets:
        ...
        if (user.is_admin(target, chan, db, bot)):
            notice("[{}]: {} is an admin and cannot be ignored.".format(
                chan, inp))
        else:
            if ignorelist and target in ignorelist:
                notice("[{}]: {} is already ignored.".format(chan, target))
            else:
                ignorelist = '{} {}'.format(target, ignorelist)
                database.set(db, 'channels', 'ignored', ignorelist, 'chan',
                             chan)

                notice("[{}]: {} has been ignored.".format(chan, target))
    return
...
def unignore(inp, notice=None, bot=None, chan=None, db=None):
    ...
    for target in targets:
        ...
        if ignorelist and target in ignorelist:
            ...
            notice("[{}]: {} has been unignored.".format(chan, target))
        else:
            notice("[{}]: {} is not ignored.".format(chan, target))
    return
...
def disabled(inp, notice=None, bot=None, chan=None, db=None):
    ...
    if disabledcommands:
        notice("[{}]: Disabled commands: Chan: {}, Global: {}".format(
            chan, disabledcommands, disabledglobalcommands))
    else:
        notice("[{}]: No commands are currently disabled.".format(chan))
    return
...
def enable(inp, notice=None, bot=None, chan=None, db=None):
    ...
    if 'all' in targets or '*' in targets:
        ...
        notice("[{}]: All commands are now enabled.".format(chan))
    else:
        for target in targets:
            if disabledcommands and target in disabledcommands:
                ...
                for commands in disabledcommands:
                    if target == commands:
                        ...
                        notice("[{}]: {} is now enabled.".format(
                            chan, target))
                    else:
                        pass
            else:
                if target in " ".join(bot.config["disabled_commands"]):
                    notice("[{}]: {} is globally disabled. Use .genable {} to enable."
                           .format(chan, target, target))
                else:
                    notice("[{}]: {} is not disabled.".format(chan, target))
    return
...
def disabledhashes(inp, notice=None, bot=None, chan=None, db=None):
    ...
    disabledhashes = database.get(db, 'channels', 'disabledhashes', 'chan',
                                  chan)
    if disabledhashes:
        notice("[{}]: Disabled hashtags: {}".format(chan, disabledhashes))
    else:
        notice("[{}]: No hashtags are currently disabled.".format(chan))
    return
    ...
    if 'all' in targets or '*' in targets:
        ...
        for target in targets:
            if disabledhashes and target in disabledhashes:
                ...
                database.set(db, 'channels', 'disabledhashes', disabledhashes,
                             'chan', chan)
                notice("[{}]: {} is now enabled.".format(chan, target))
            else:
                notice("[{}]: {} is not disabled.".format(chan, target))
    return
...
def showfloods(inp, chan=None, notice=None, db=None):
    ...
    if flood:
        notice("[{}]: Flood: {} messages in {} seconds".format(
            chan,
            flood.split()[0],
            flood.split()[1]))
    else:
        notice("[{}]: Flood protection is disabled.".format(chan))

    cmdflood = database.get(db, 'channels', 'cmdflood', 'chan', chan)
    if cmdflood:
        notice("[{}]: Command Flood: {} commands in {} seconds".format(
            chan,
            cmdflood.split()[0],
            cmdflood.split()[1]))
    else:
        notice("[{}]: Command Flood protection is disabled.".format(chan))
    return
...
def flood(inp, conn=None, chan=None, notice=None, db=None):
    ...
    if len(inp) == 0:
        ...
        if floods:
            ...
            notice("[{}]: Flood is disabled.".format(chan))
            notice(flood.__doc__)
    elif "disable" in inp:
        database.set(db, 'channels', 'flood', None, 'chan', chan)
        notice("[{}]: Flood Protection Disabled.".format(chan))
    else:
        ...
        notice("[{}]: Flood Protection limited to {} messages in {} seconds."
...
def cmdflood(inp, conn=None, chan=None, notice=None, db=None):
    ...
    if len(inp) == 0:
        ...
        if floods:
            ...
            notice("[{}]: CMD Flood is disabled.".format(chan))
            notice(cmdflood.__doc__)
    elif "disable" in inp:
        database.set(db, 'channels', 'cmdflood', None, 'chan', chan)
        notice("[{}]: Command Flood Protection Disabled.".format(chan))
    else:
        flood_num = inp.split()[0]
        flood_duration = inp.split()[1]
        floods = '{} {}'.format(flood_num, flood_duration)
        database.set(db, 'channels', 'cmdflood', floods, 'chan', chan)
        notice("[{}]: Command Flood Protection limited to {} commands in {} seconds."
               .format(chan, flood_num, flood_duration))
    return
...
def badwords(inp, notice=None, bot=None, chan=None, db=None):
    ...
    if len(inp) == 0 or 'list' in inp:
        ...
        if badwordlist:
            notice("[{}]: Bad words: {}".format(chan, badwordlist))
        else:
            notice("[{}]: No bad words in list.".format(chan))
        ...
        if 'add' in command:
            for target in targets:
                if badwordlist and target in badwordlist:
                    notice("[{}]: {} is already a bad word.".format(
                        chan, target))
                else:
                    if len(target) < 3:
                        notice("[{}]: badwords must be longer than 3 characters. ({})"
                               .format(chan, target))
                    else:
                        badwordlist = '{} {}'.format(target, badwordlist)
                        database.set(db, 'channels', 'badwords', badwordlist,
                                     'chan', chan)
                        notice("[{}]: {} has been added to the bad word list."
                               .format(chan, target))
        elif 'del' in command:
            if 'all' in targets or '*' in targets:
                database.set(db, 'channels', 'badwords', '', 'chan', chan)
                notice("[{}]: All bad words have been removed.".format(chan))
            else:
                for target in targets:
                    if badwordlist and target in badwordlist:
                        badwordlist = " ".join(
                        ...
                        database.set(db, 'channels', 'badwords', badwordlist,
                                     'chan', chan)
                        notice("[{}]: {} is no longer a bad word.".format(
                            chan, target))
                    else:
                        notice("[{}]: {} is not a bad word.".format(
                            chan, target))
    else:
        notice(badwords.__doc__)
...
def trim(inp, conn=None, chan=None, notice=None, db=None):
    ...
    if len(inp) == 0:
        ...
        if trimlength:
            notice("[{}]: Trim output set to {} characters.".format(
                chan, trimlength))
        else:
            notice("[{}]: Trim is disabled.".format(chan))
            notice(trim.__doc__)
    elif "disable" in inp or "0 " in inp:
        database.set(db, 'channels', 'trimlength', None, 'chan', chan)
        notice("[{}]: Trim Disabled.".format(chan))
    else:
...
def speak(inp, conn=None, chan=None, notice=None):
    ...
    for user in users:
        out = "MODE %s -m" % (chan)
        conn.send(out)
        notice("hey %s you can speak in %s again" % (user, chan))
...
def mode_cmd(mode, text, inp, chan, conn, notice):
    ...
    for target in targets:
        if notice is None:
            conn.send("MODE {} {} {}".format(chan, mode, target))
            pass
        else:
            notice("Attempting to {} {} in {}...".format(text, target, chan))
            conn.send("MODE {} {} {}".format(chan, mode, target))
    return
...
def bans(inp, notice=None, bot=None, chan=None, db=None):
    ...
    if bans: notice("[{}]: Bans are: {}".format(chan, bans))
    else: notice("[{}]: No nicks/hosts are in the banlist.".format(chan))
    return
...
def ban(inp, conn=None, chan=None, notice=None, db=None, nick=None, bot=None):
    ...
    if conn.nick in inp_nick or bot.config['owner'] == inp_nick:
        ...
        conn.send("KICK {} {} :{}".format(chan, target, reason))
        return
    ...
    notice("Attempting to ban {} in {}...".format(target, chan))
    conn.send("MODE {} {} {}".format(chan, mode, target))
    conn.send("KICK {} {} :{}".format(chan, inp_nick, reason))

    if timer > 0:
        notice("{} will be unbanned in {} seconds".format(target, timer))
...
def unban(inp, conn=None, chan=None, notice=None, db=None):
    ...
    if '@' in target and not '!' in target: target = '*!*{}'.format(target)
        ...
        notice("[{}]: Everyone unbanned".format(chan))
        database.set(db, 'channels', 'bans', '', 'chan', chan)
        return
    notice("Attempting to unban {} in {}...".format(target, chan))
...
def kick(inp, chan=None, conn=None, notice=None, nick=None, bot=None):
    ...
    notice("Attempting to kick {} from {}...".format(target, chan))
    conn.send("KICK {} {} :{}".format(chan, target, reason))
    return
...
def invite(
    ...
    def nestedinvite(user, throttle, sleeptime, users):
        conn.send("INVITE {} {}".format(user, chan))
        notice("Inviting {} to {}...".format(user, chan))
        sleep(sleeptime)
...
def remove(inp, chan=None, conn=None):
    ...
    print(inp)
    message = inp
    conn.send("REMOVE {} :{}".format(chan, message))
    return
